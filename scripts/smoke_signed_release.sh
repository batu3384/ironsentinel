...
export AEGIS_RELEASE_PRIVATE_KEY_B64
AEGIS_RELEASE_PRIVATE_KEY_B64="$(tr -d '\n' < "$PRIVATE_KEY_PATH")"
export AEGIS_EXTERNAL_ATTESTATION_PROVIDER="smoke-ci"
export AEGIS_EXTERNAL_ATTESTATION_SOURCE_URI="https://example.invalid/ironsentinel/signed-smoke"
...
(cd "$ROOT" && APPSEC_BUNDLE_LOCK_PATH="$LOCK_PATH" AEGIS_DIST_DIR="$DIST_ROOT" go run ./cmd/ironsentinel runtime release verify --lang en --version "$VERSION" --require-signature --require-attestation --require-external-attestation)
